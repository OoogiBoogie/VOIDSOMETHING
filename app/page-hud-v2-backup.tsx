...
import HUDv2 from "@/components/hud-v2/HUDv2"
...
export default function VOIDMetaverse() {
  ...
  const [hudV2Enabled, setHudV2Enabled] = useState(true) // Toggle for testing HUD v2
  ...
  useEffect(() => {
    (window as any).resetIntro = () => {
      ...
      localStorage.removeItem("void_wallet_connected")
      setIntroComplete(false)
      setWarningAccepted(false)
      setWalletConnected(false)
      setGameStarted(false)
      console.log("🔄 Intro reset complete! Warning screen will appear now.")
    }
  ...
  useEffect(() => {
    ...
    const handleKeyPress = (e: KeyboardEvent) => {
      ...
      if ((e.key === "r" || e.key === "R") && e.shiftKey) {
        ...
        setWalletConnected(false)
        setGameStarted(false)
        console.log("🔄 Intro reset! Warning screen will appear now.")
      }
    }
    ...
    >
      ...
      {introComplete === true && walletConnected === false && (
        <WalletConnectionModal
          ...
          onSkip={() => {
            // Allow user to skip wallet connection and continue as guest
      ...
      {introComplete === true && walletConnected === true && !userProfile && <UserProfileSetup onComplete={handleProfileComplete} />}
      ...
      {/* NEW HUD v2 - Glassmorphism Design */}
      {hudV2Enabled && gameStarted && userProfile && (
        <HUDv2
          ...
          onPhoneOpen={() => setPhoneOpen(!phoneOpen)}
          onMapOpen={() => setMapOpen(!mapOpen)}
          onMarketOpen={() => setMarketplaceOpen(!marketplaceOpen)}
          onDashboardOpen={() => setDashboardOpen(!dashboardOpen)}
          onRealEstateOpen={() => setLandInventoryOpen(!landInventoryOpen)}
          onVoiceChatOpen={() => setVoiceChatOpen(!voiceChatOpen)}
          onMusicOpen={() => setJukeboxOpen(!jukeboxOpen)}
          onVoidHubOpen={() => setVoidHubOpen(!voidHubOpen)}
          showEconomyTicker={true}
          voidPrice={0.0042}
          voidChange={5.2}
          psxPrice={0.0089}
          psxChange={-2.1}
      ...
      >
        <div className="p-6 space-y-4">
          ...
          <div className="mt-8 pt-4 border-t border-[#9ccc00]/20">
            <button
              onClick={() => { setHudV2Enabled(!hudV2Enabled); }}
              className="w-full text-left px-4 py-3 bg-black/50 border-2 border-purple-500/30 hover:border-purple-500 rounded text-white font-mono transition-colors"
            >
              <span className="text-purple-400">H</span> - HUD v2 (Test): {hudV2Enabled ? 'ON' : 'OFF'}
            </button>
            
            <button
              onClick={() => { setCrtEnabled(!crtEnabled); }}
              className="w-full text-left px-4 py-3 mt-2 bg-black/50 border-2 border-cyan-500/30 hover:border-cyan-500 rounded text-white font-mono transition-colors"
            >
